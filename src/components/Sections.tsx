...
function SectionTag({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-6 inline-flex items-center gap-3">
      ...
      <span className="text-[11px] font-medium tracking-[0.4em] text-gold/70 uppercase">{children}</span>
    </div>
  );
}

/* ── Ambient section particles — sparse, elegant ── */
    ...
    <section id="servicios" className="relative py-32">
      ...
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ show: { transition: { staggerChildren: 0.12 } } }} className="mb-16 max-w-3xl">
          ...
          <motion.h2 variants={fadeUp} className="font-display text-[clamp(2.2rem,5vw,3.8rem)] leading-[1.05]">
            Tecnología de show.{" "}<span className="text-gradient-gold">Ejecución impecable.</span>
          </motion.h2>
        </motion.div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {services.map((s, i) => (
            <motion.article key={s.title} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.9, delay: i * 0.15, ease: EASE }}
              className="card-cinematic group relative flex flex-col overflow-hidden rounded-2xl border border-gold/10 bg-[#0B0B0B]"
            >
              {/* Image with cinematic hover glow */}
              <div className="relative aspect-[4/3] overflow-hidden img-cinematic">
                ...
                <div className="absolute left-5 top-5 flex h-11 w-11 items-center justify-center rounded-full glass glass-reflect transition-all duration-700 group-hover:shadow-[0_0_20px_rgba(201,169,110,0.15)]">
                  <s.icon className="h-5 w-5 text-gold" />
                </div>
              </div>
              {/* Content */}
              <div className="relative z-[2] flex flex-1 flex-col p-7">
                ...
                <div className="mt-6 flex items-center gap-2 text-xs tracking-widest text-foreground/50 transition-all duration-700 group-hover:text-gold group-hover:gap-3">
  ...
  return (
    <section id="experience" className="relative overflow-hidden py-32">
      <div className="absolute inset-0 -z-10">
        <img src={djImg} alt="" className="h-full w-full object-cover opacity-15" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#050505] via-[#050505]/90 to-[#050505]" />
      </div>
      <AmbientParticles count={5} />
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ show: { transition: { staggerChildren: 0.12 } } }} className="mx-auto max-w-4xl text-center">
        ...
        <div className="mt-20 grid grid-cols-2 gap-[1px] overflow-hidden rounded-2xl border border-gold/10 bg-gold/10 md:grid-cols-3">
          {items.map((it, i) => (
            <motion.div key={it.t} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: i * 0.08 }}
              className="group relative flex flex-col gap-4 bg-[#0B0B0B] p-8 transition-all duration-700 hover:bg-[#0e0e0e]"
            >
              {/* Icon with hover glow */}
              <div className="flex h-12 w-12 items-center justify-center rounded-full border border-gold/20 bg-gold/5 transition-all duration-700 group-hover:border-gold/40 group-hover:bg-gold group-hover:text-[#050505] group-hover:shadow-[0_0_24px_rgba(201,169,110,0.25)]">
                <it.icon className="h-5 w-5 text-gold transition-colors duration-700 group-hover:text-[#050505]" />
              </div>
              <p className="font-display text-xl leading-tight">{it.t}</p>
              {/* Bottom reveal line */}
...
export function Events() {
  ...
  return (
    <section id="eventos" className="relative py-32">
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ show: { transition: { staggerChildren: 0.1 } } }} className="mb-16 max-w-3xl">
        ...
        <div className="grid auto-rows-[220px] grid-cols-2 gap-3 md:grid-cols-4">
          {events.map((e, i) => (
            <motion.a href={WHATSAPP} target="_blank" rel="noreferrer" key={e.name}
              initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8, delay: i * 0.06 }}
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 transition-transform duration-700 group-hover:-translate-y-1">
                <div className="text-[10px] uppercase tracking-[0.35em] text-gold/70 opacity-0 transition-all duration-700 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0">Consultar →</div>
                <div className="font-display text-xl sm:text-2xl">{e.name}</div>
              </div>
            </motion.a>
          ))}
        </div>
...
export function Gallery() {
  const imgs = [
    { src: g2, h: "row-span-2", alt: "Metallic confetti rain" },
    { src: g1, h: "", alt: "Fireworks display" },
    { src: g3, h: "", alt: "Night pyrotechnic show" },
    { src: weddingImg, h: "row-span-2", alt: "Wedding cold sparks" },
    { src: g4, h: "", alt: "Confetti explosion" },
  ];
  return (
    <section id="galeria" className="relative py-32">
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid auto-rows-[200px] grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {imgs.map((im, i) => (
            <motion.figure key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: (i % 4) * 0.08 }}
...
export function Reviews() {
  ...
  return (
    <section id="resenas" className="relative overflow-hidden py-32">
      ...
      {/* Warm ambient glow */}
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[60%] w-[40%] rounded-full opacity-[0.04] blur-[100px] animate-warm-pulse" style={{ background: "var(--gold)" }} />
      <div className="mx-auto max-w-7xl px-6">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ show: { transition: { staggerChildren: 0.12 } } }} className="mb-16 max-w-3xl">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {reviews.map((r, i) => (
            <motion.figure key={r.name} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.9, delay: i * 0.12 }}
              className="group relative flex flex-col gap-6 rounded-2xl glass glass-reflect p-8 transition-all duration-700 hover:border-gold/30 hover:-translate-y-2 hover:shadow-[0_0_40px_rgba(201,169,110,0.06)]"
            >
              {/* Stars with subtle glow */}
              <div className="flex gap-1">
                {Array.from({ length: r.stars }).map((_, k) => (
                  <Star key={k} className="h-4 w-4 fill-gold text-gold drop-shadow-[0_0_4px_rgba(201,169,110,0.3)]" />
                ))}
              </div>
              <blockquote className="text-base leading-relaxed text-foreground/80 font-light">"{r.text}"</blockquote>
              <figcaption className="mt-auto flex items-center gap-3 border-t border-gold/10 pt-5">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-gold text-sm font-bold text-[#050505] shadow-[0_0_16px_rgba(201,169,110,0.2)] transition-shadow duration-700 group-hover:shadow-[0_0_24px_rgba(201,169,110,0.35)]">{r.initials}</div>
                <div>
...
export function FinalCTA() {
  return (
    <section id="contact" className="relative overflow-hidden py-40">
      <div className="absolute inset-0 -z-10">
        <img src={heroImg} alt="" className="h-full w-full object-cover opacity-20" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#050505] via-[#050505]/80 to-[#050505]" />
        <div className="absolute inset-0 opacity-30" style={{ background: "var(--gradient-radial-gold)" }} />
      </div>
      {/* Breathing warm glow */}
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[50%] w-[35%] rounded-full opacity-[0.06] blur-[120px] animate-breathe" style={{ background: "var(--gold)" }} />
      <div className="relative mx-auto max-w-4xl px-6 text-center">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={{ show: { transition: { staggerChildren: 0.15 } } }}>
          <motion.h2 variants={fadeUp} className="font-display text-[clamp(2.4rem,6vw,4.5rem)] leading-[1.05] text-cinema">
            Tu evento merece{" "}<span className="text-gradient-gold">un momento cinemático.</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="mx-auto mt-8 max-w-xl text-lg text-muted-foreground font-light">
...
export function Footer() {
  return (
    <footer className="relative border-t border-gold/8 py-16">
      {/* Subtle top glow */}
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 md:grid-cols-3">
        <div>
          <img src={logoSrc} alt="Eclipse Eventos" className="h-12 w-auto brightness-0 invert opacity-70" />
          <p className="mt-5 max-w-xs text-sm text-muted-foreground font-light">
